...
describe("Pizza Routes", () => {
    ...
    describe("GET /api/pizza/admin", () => {
        it("should return 200 if user is registered successfully", async () => {
            const res = await request(app).get("/api/pizza/admin").set("Authorization", `Bearer ${token}`);
            expect(res.status).toBe(200);
        });

        it("should return 500 if user is not authenticated", async () => {
            const res = await request(app).get("/api/pizza/admin");
            expect(res.status).toBe(500);
            expect(res.body.error).toBe("Access denied");
        });
    })
